...
(function (global, factory) {
  ...
  var setCenter$1 = function setCenter(el, a, b) {
    return coordinates.apply(void 0, _toConsumableArray(flatten(a, b))).slice(0, 2).forEach(function (value, i) {
      return el.setAttribute(NodeAttributes.ellipse[i], value);
    });
  };
  ...
  var setPoints$1 = function setPoints(element, a, b, c, d) {
  ...
  var appendPathItem = function appendPathItem(el, command) {
    ...
    return el.setAttribute("d", "".concat(getD(el)).concat(command).concat(flatten.apply(void 0, args).join(" ")));
  };
  ...
  var clear = function clear(element) {
    return element.removeAttribute("d");
  };
  var getCommands = function getCommands(element) {
    return parsePathCommands(getD(element));
  };
  var setCommands = function setCommands(element, commandsObject) {
    return commandsObject.forEach(function (el) {
      return appendPathItem(element, el.command, el.values);
    });
  };
  var setString = function setString(element, commandsString) {
    return element.setAttribute("d", commandsString);
  };
  var setters = {
    "string": setString,
    "object": setCommands
  };
  var noClearSet = function noClearSet(element) {
    if ((arguments.length <= 1 ? 0 : arguments.length - 1) === 1) {
      var typ = _typeof(arguments.length <= 1 ? undefined : arguments[1]);
      if (setters[typ]) {
        setters[typ](element, arguments.length <= 1 ? undefined : arguments[1]);
      }
    }
  };
  var clearAndSet = function clearAndSet(element) {
    if ((arguments.length <= 1 ? 0 : arguments.length - 1) === 1) {
      var typ = _typeof(arguments.length <= 1 ? undefined : arguments[1]);
      if (setters[typ]) {
        clear(element);
  ...
  var setPoints$2 = function setPoints(element) {
    for (var _len = arguments.length, args = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
      args[_key - 1] = arguments[_key];
    }
    return element.setAttribute("points", stringifyArgs.apply(void 0, args)[0]);
  };
  var polys = {
    polyline: {
      args: stringifyArgs,
  ...
  var Spec = Object.assign({}, circle, ellipse, line, path, rect, style, text, svg, maskTypes, polys);
  var ManyElements = {
  ...
  var constructor = function constructor(nodeName) {
    var _NodeSpec$nodeName, _NodeSpec$nodeName2;
    for (var _len = arguments.length, args = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
      args[_key - 1] = arguments[_key];
    }
    var element = win.document.createElementNS(NS, Nodes[nodeName].nodeName);
    RequiredAttributes(element, nodeName);
    (_NodeSpec$nodeName = Nodes[nodeName]).init.apply(_NodeSpec$nodeName, [element].concat(args));
    (_NodeSpec$nodeName2 = Nodes[nodeName]).args.apply(_NodeSpec$nodeName2, args).forEach(function (v, i) {
    ...
    Object.keys(Nodes[nodeName].methods).forEach(function (methodName) {
      return Object.defineProperty(element, methodName, {
        value: function value() {
          var _NodeSpec$nodeName$me;
          return (_NodeSpec$nodeName$me = Nodes[nodeName].methods[methodName]).call.apply(_NodeSpec$nodeName$me, [bound, element].concat(Array.prototype.slice.call(arguments))) || element;
        }
      });
    });
