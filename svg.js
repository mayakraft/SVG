...
(function (global, factory) {
  ...
  var Spec = Object.assign({}, circle, ellipse, line, path, polygon, rect, style, text, svg, maskTypes);
  ...
  [[["svg", "defs", "g"].concat(Nodes.v, Nodes.t), ManyElements.presentation], [["filter"], ManyElements.effects], [Nodes.cT.concat("text"), ManyElements.text], [Nodes.cF, ManyElements.effects], [Nodes.cG, ManyElements.gradient]].forEach(function (pair) {
  ...
  (_NodeList$v = Nodes.v).push.apply(_NodeList$v, _toConsumableArray(Object.keys(nodes)));
  ...
  Object.assign(Nodes$1, Spec, nodes);
  Object.keys(Nodes).forEach(function (key) {
    return Nodes[key].filter(function (nodeName) {
      return Nodes$1[nodeName] === undefined;
    }).forEach(function (nodeName) {
      Nodes$1[nodeName] = {};
    });
  });
  ...
  Object.keys(Nodes$1).forEach(function (key) {
    if (!Nodes$1[key].nodeName) {
      Nodes$1[key].nodeName = key;
    }

    if (!Nodes$1[key].init) {
      Nodes$1[key].init = passthrough;
    }

    if (!Nodes$1[key].args) {
      Nodes$1[key].args = passthrough;
    }

    if (!Nodes$1[key].methods) {
      Nodes$1[key].methods = {};
    }

    if (!Nodes$1[key].attributes) {
      Nodes$1[key].attributes = NodeAttributes[key] || [];
    }
  });
  ...
  var assign = function assign(groups, Methods) {
    groups.forEach(function (n) {
      return Object.keys(Methods).forEach(function (method) {
        Nodes$1[n].methods[method] = function () {
  ...
  var N = Nodes;
  assign(flatten(N.v, N.g, N.s), methods$4);
  assign(flatten(N.t, N.v, N.g), methods$5);
  ...
  var constructor = function constructor(nodeName) {
    ...
    var element = win.document.createElementNS(NS, Nodes$1[nodeName].nodeName);
    RequiredAttributes(element, nodeName);

    (_Spec$nodeName = Nodes$1[nodeName]).init.apply(_Spec$nodeName, [element].concat(args));

    (_Spec$nodeName2 = Nodes$1[nodeName]).args.apply(_Spec$nodeName2, args).forEach(function (v, i) {
      if (Nodes$1[nodeName].attributes[i] != null) {
        element.setAttribute(Nodes$1[nodeName].attributes[i], v);
      }
    });

    Nodes$1[nodeName].attributes.forEach(function (attribute) {
    ...
    Object.keys(Nodes$1[nodeName].methods).forEach(function (methodName) {
      return Object.defineProperty(element, methodName, {
        value: function value() {
          var _Spec$nodeName$method;

          return (_Spec$nodeName$method = Nodes$1[nodeName].methods[methodName]).call.apply(_Spec$nodeName$method, [bound, element].concat(Array.prototype.slice.call(arguments)));
        }
      });
    });
